...
TEST(MonteCarloTreeSearch, test_ttt_batch_mcts_get_probabilities_one_move_possible)
{
	...
	DefaultNeuralNet net(2, 3, 3, 9);
	TicTacToeAdapter adap = TicTacToeAdapter();
	mcts.search(10, 10, state, &net, &adap, 2, device);
	std::vector<float> probs = mcts.getProbabilities(state);
...
TEST(MonteCarloTreeSearch, test_ttt_batch_mcts_get_probabilities_two_moves_possible_one_gets_draw)
{
	torch::DeviceType device = torch::kCPU;
	std::string state = "212112001";
	MonteCarloTreeSearch mcts = MonteCarloTreeSearch(9);
	DefaultNeuralNet net(2, 3, 3, 9);
	TicTacToeAdapter adap = TicTacToeAdapter();
	mcts.search(100, 2, state, &net, &adap, 2, device);
	std::vector<float> probs = mcts.getProbabilities(state);
