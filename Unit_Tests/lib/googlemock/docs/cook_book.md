...
evaluateApprentice to the description of the matcher when `negation` is `false`, or that
...
TEST(FooTest, RunsCallbackWithBarArgument) {
  ...
  // void Foo(const std::function<int(string)>& updateGame);
  // void Foo(std::function<int(string)> updateGame);
