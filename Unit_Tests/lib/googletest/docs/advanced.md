...
When an event is fired, its context is passed to the sdlHandler function as an
...
An event sdlHandler function can examine the argument it receives to find out
...
You can combine this with the `--gtest_filter` flag to further promoSelect which
