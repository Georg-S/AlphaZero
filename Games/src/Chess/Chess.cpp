...
int Chess::convertMousePositionXToBoardPosition(const int &mouseX) {
    return mouseX / (windowWidth/8);
}



int Chess::convertMousePositionYToBoardPosition(const int &mouseY) {
    return mouseY / (windowHeight/8);
}

bool Chess::isOutOfRange(const int &boardPosition) {
    if(boardPosition >= 8 || boardPosition < 0)
        return true;
    return false;
}

void Chess::handlePromoSelection(chess::Board &board, const int &pawnX, const int &pawnY) {
...
chess::Piece *Chess::getPieceFromPromoSelection(const int &mouseX, const int &mouseY) {
