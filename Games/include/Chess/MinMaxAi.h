...
namespace chess {
    ...
    private:
        ...
        void replacePawnsInRowWithQueens(chess::Board &board, const int &row);
        std::vector<chess::Move> getAllPossibleMoves(chess::Board &board, const chess::PieceColor &color);
        void getAllPossibleMovesForPiece(chess::Board &board, const int &fromX, const int &fromY, std::vector<chess::Move> &moves);
